...
![Creating Dynamic Interfaces Using JavaScript](Images/U0901.png)
...
![Resolution-dependent layouts](Images/U0902.png)
...
![Resolution-dependent layouts](Images/U0903.png) ![The UX Magazine website layout optimized for a maximized browser window at 800600](Images/0901.png)

**图 9.1。UX 杂志网站布局针对 800600** 的最大化浏览器窗口进行了优化

![The UX Magazine website layout optimized for a maximized browser window at 1024768](Images/0902.png)

**图 9.2。UX 杂志网站布局针对 1024768** 的最大化浏览器窗口进行了优化

![The White Pages website layout optimized for narrower browser windows (less than 1200px wide)](Images/0903.png)

**图 9.3。针对较窄的浏览器窗口(宽度小于 1200 像素)优化的白页网站布局**

![The White Pages website layout optimized for wider browser windows (greater than 1200px wide)](Images/0904.png)
...
![The sample page, optimized for 800600](Images/0905.png)

**图 9.5。针对 800600** 优化的示例页面

![The sample page, optimized for 1024768](Images/0906.png)
...
![The default view of the modular layout example](Images/0907.png)

**图 9.7。模块化布局示例的默认视图**

图 9-8 显示了一个修改后的界面视图，在用户通过折叠、重组和移动的组合按照自己的喜好重新排列模块后。

![A user-defined view of the modular layout example](Images/0908.png)
  ...
  for (var i in modules)
  {
    ...
    for (var i = 0; i < h2s.length; i++)
    {
      ...
      newA.onclick = clickExpandCollapse;
        ...
        newImg.setAttribute("src", "images/min_max.gif");
...
![The two icons for expanding and collapsing combined in one image](Images/0909.png)
...
![The calendar module in its default, expanded view](Images/0910.png)

**图 9.10。默认扩展视图中的日历模块**

![The calendar module after it has been collapsed](Images/0911.png)
...
![The mouse cursor changes appearance when it is hovered over a module's title bar.](Images/0912.png)
...
你可以在[图 9-13](#the_ghost_copy_of_a "Figure 9.13. The ghost copy of a module next to its original") 中看到结果。

为了跟踪按钮被按下时用户移动鼠标的位置，我们需要向整个文档添加一个 mousemove 侦听器。我们添加了一个 mouseup 侦听器来告诉我们用户何时释放了鼠标按钮。

![The ghost copy of a module next to its original](Images/0913.png)
...
![The ghost marker showing where the dragged module will be inserted](Images/0914.png)
...
![Dragging a module to another module area](Images/0915.png)

**图 9.15。将一个模块拖到另一个模块区**

![Dragging a module to another module area](Images/U0904.png)
