...
该实现的输出如图 [9-11](#Fig11) 所示。请注意，结果再次与第 5 章[的原始版本几乎相同。这是意料之中的，因为从根本上说，这两种实现的代码是相同的；区别在于如何将属性分配给标记。您喜欢哪种方法由您自己决定，但是使用预构建的库来按照通用的设计模式创建布局，而不需要每次都创建自己的解决方案是非常有用的。](05.html)
