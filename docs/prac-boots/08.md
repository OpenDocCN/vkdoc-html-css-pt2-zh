...
您将从标准的空 Twitter 引导页面开始(清单 [8-1](#PC1) )。
...
有些情况下，您只有一行来显示表单的所有输入控件。下面是这种表单的一个例子(图 [8-7](#Fig7) )。
...
如果您保存此文件并在浏览器上重新加载页面，您将看到以下内容(图 [8-13](#Fig13) )。
...
如果您保存前面的代码并在浏览器上重新加载页面，您将看到以下内容(图 [8-20](#Fig20) )。
