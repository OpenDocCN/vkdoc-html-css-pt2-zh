...
![image](img/sq.jpg) **注意**在本章的代码清单中，您可能会注意到父容器的`display`值可能被设置为`-ms-flexbox`或`-ms-grid`。目前，只知道这些是布局技术。flexbox 非常适合将它的孩子一个接一个地排列起来，在这种情况下，网格用于将他们堆叠在一起。我们将在第 7 章中详细讨论这些和其他布局技术。
